# Mo Hayoung Portfolio

GitHub Pages로 배포하는 개인 포트폴리오 사이트입니다.  
대문에서 `About`, `Resume`, `Portfolio`로 이동하고, About 페이지 안에서 접히는 이력서를 확인할 수 있습니다.

- Site: https://hayoungmo.github.io/
- About: https://hayoungmo.github.io/about.html
- Resume: https://hayoungmo.github.io/about.html#resume
- Portfolio: https://hayoungmo.github.io/portfolio.html
- GitHub: https://github.com/HayoungMo

## Page Structure

| Page | Role |
| --- | --- |
| `index.html` | 대문 페이지. 큰 타이포와 `M H Y` 워터마크로 첫인상을 만들고 Resume/Portfolio로 이동합니다. |
| `about.html` | 자기소개, 개발 방식, 기술 스택, 접이식 이력서를 정리합니다. |
| `portfolio.html` | AI/Fullstack 프로젝트, 영상 영역, GitHub README, PDF 링크를 정리합니다. |

## Project Groups

### AI Projects

- Final Project: BidMatch - AI 기반 나라장터 입찰 공고 추천 서비스
- Deep Learning: CNN 기반 가구 이미지 카테고리 분류
- Machine Learning: 학생 성적 예측 및 위험군 분류
- LLM/RAG: 이미지 분류와 RAG 기반 가구 추천 챗봇

### Fullstack

- PlusHome

<!-- ## Assets

`assets/` 폴더에는 이력서, 프로젝트 PDF, 프로필 이미지를 둡니다.

```text
assets/
  resume-backend.pdf
  plushome-full.pdf
  plushome-mini.pdf
  ML PRJ_MHY.pdf
  DL PRJ_MHY.pdf
  LLM PRJ_MHY.pdf
  profile.jpeg
```

PDF 파일을 같은 이름으로 교체하면 HTML 링크는 그대로 두고 PDF만 commit/push하면 됩니다.

## Video Embed

YouTube에 영상을 일부 공개로 업로드한 뒤 `portfolio.html`의 `video-frame pending` 영역을 iframe으로 교체합니다.

```html
<iframe
  class="video-frame"
  src="https://www.youtube.com/embed/영상ID"
  title="Project demo"
  allowfullscreen>
</iframe>
```

## Deploy

로컬에서 수정한 뒤 아래 순서로 반영합니다.

```bash
git status -sb
git add .
git commit -m "Update portfolio"
git push origin main
```

`main` 브랜치에 push하면 GitHub Actions가 `index.html`, `about.html`, `portfolio.html`, `style.css`, `assets` 폴더를 GitHub Pages로 배포합니다.-->
